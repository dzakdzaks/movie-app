...
class MoviesRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            when (val result = fetchNowPlayingMoviesUseCase(page = page)) {
                WrapperResponse.Empty -> {
                    MediatorResult.Error(Throwable("There is no playing movies available."))
                }
                ...
                is WrapperResponse.Success -> {
                    ...
                    db.withTransaction {
                        if (loadType == LoadType.REFRESH) {
                            ...
                            db.getMoviesDao().clearAllMovies()
                        }
